...
"""
    ...
    func listAllProperties() throws {
        ...
        let t = try Self.parse(content: content[range], range: range)
        print(t)
...
//            print(properties)
    ...
    private static func parse(content: Substring, range: Range<String.Index>) throws -> [Any] {
        ...
        while let result = try regex.firstMatch(in: currentContent[currentIndex..<endIndex]) {
            ...
            if let idString = result.id {
                ...
                let comment = XProjComment(comment: commentString)
                ...
                let objectBodyStartIndex = currentContent.index(after: objectRange.lowerBound)
                let objectBodyEndIndex = currentContent.index(before: objectRange.upperBound)
                let objectBodyRange = objectBodyStartIndex..<objectBodyEndIndex
                ...
                let arrayBodyStartIndex = currentContent.index(after: arrayRange.lowerBound)
                let arrayBodyEndIndex = currentContent.index(before: arrayRange.upperBound)
                let arrayBodyRange = arrayBodyStartIndex..<arrayBodyEndIndex
